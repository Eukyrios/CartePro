...
| `brand/` | L'identité : le logotype vectorisé, le monogramme, la flèche. | tout le monde |
...
| `account/` | Le compte connecté : contexte, comptes de démonstration, solde. | tout le monde |
...
| `espace/` | L'espace salarié : solde, coups de cœur, réseau, historique, paiement. | `app/espace/**` |
...
pliage de texte. Un hook qui n'a qu'un client vit à côté de lui (`espace/useLedger`,
`espace/useQrToken`, `account/useBalance`).
