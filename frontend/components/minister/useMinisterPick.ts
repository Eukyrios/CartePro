...
export type MinisterPick = ApiPartner & { mot: string };

/**
 * Le coup de cœur du Ministre, depuis la base.
 *
 ...
 * `coups_de_coeur`, avec le mot du Ministre, un horodatage et un statut —
 ...
 */
export function useMinisterPick(): {
  pick: MinisterPick | null;
  loaded: boolean;
} {
  const [pick, setPick] = useState<MinisterPick | null>(null);
  const [loaded, setLoaded] = useState(false);

  useEffect(() => {
    let cancelled = false;
    api<{ coup_de_coeur: MinisterPick | null }>(
      "/api/partenaires/coup-de-coeur",
    )
